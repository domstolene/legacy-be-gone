...
6. Update any Dockerfiles to use gradle commands instead of maven commands. For example, update Dockerfile to use `./gradlew build` instead of `mvn clean install`. Ensure source paths are relative to the Github Actions workspaces context, for example `COPY src ./src` instead of `COPY ../src ./src`.
7. Update documentation to use gradle commands instead of maven commands. You can find documentation by searching for maven commands: `rg "mvn " --files-with-matches`. For example, update README.md to use gradle commands for build and test.
8. Search for other places where maven is mentioned: `rg maven --files-with-matches`, and update to gradle if relevant. For example, update dependabot to use gradle instead of maven.
9. Delete any maven specific configuration files that are no longer needed, for example `pom.xml`, `mvnw` and `mvnw.cmd` wrapper scripts, and `.mvn` directory.
