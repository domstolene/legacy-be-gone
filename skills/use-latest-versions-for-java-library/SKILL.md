...
2. Find latest version of the dependency in maven central:

   ```shell
   curl https://repo.maven.apache.org/maven2/<groupId>/<artifactId>/ --silent \
      | grep "a href" \
   ...
   For example, org.mockito:mockito-core:

   ```shell
   curl https://repo.maven.apache.org/maven2/org/mockito/mockito-core/ --silent \
      | grep "a href" \
      | grep -v "maven-metadata" \
      | sed -E 's|.*href="([^"]+)".*|\1|g' \
      | sort -rV
